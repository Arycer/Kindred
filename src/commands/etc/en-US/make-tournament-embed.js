...
async function make_embed (callback) {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'The game has ended!', iconURL: 'https://i.imgur.com/hGG9kLU.png' })
        .setTitle(callback.title)
        .setDescription(`Map: ${await get_map_name(callback.map, 'en-US')} - ${time(new Date(callback.timestamp), TimestampStyles.ShortDateTime)}`)
        .setColor('#5d779d')
