...
async function embed_profile(username, requester) {
    const profile = new Profile();
    await profile.init(username);
    var m_text = ``;
    for (var i = 0; i < 3; i++) {
        if (profile.mastery.champions[i].text) {
            m_text += `${i+1}. ${profile.mastery.champions[i].text}\n`;
        } else {
            m_text += `${i+1}. No hay datos para mostrar.\n`;
        }
    }
    var r_field = {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: 'Nivel', value: `${profile.summoner_data.summoner_level}`, inline: true },
                   { name: 'Puntos de maestría', value: `${profile.mastery.score} puntos en total`, inline: true },
