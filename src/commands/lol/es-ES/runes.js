...
async function execute(interaction) {
    ...
    if (!champ) return interaction.followUp({ embeds: [error('es-ES', 'champ-not-found', interaction.user.tag)] });
    ...
    captureAll([{
        ...
        if (!replied) return interaction.followUp({ embeds: [error('es-ES', 'runes-not-found', interaction.user.tag)] });
    }, 15000);
}
