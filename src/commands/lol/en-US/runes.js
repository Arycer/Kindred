...
async function execute(interaction) {
    var champion = interaction.options.getString('campeón') || interaction.options.getString('champion');
    var position = interaction.options.getString('posición') || interaction.options.getString('position');
    ...
    if (!champ) return interaction.followUp({ embeds: [error('en-US', 'champ_not_found', interaction.user.tag)] });
    ...
    captureAll([{
        ...
        results.forEach(async (result) => {
            ...
            var embed = new EmbedBuilder()
                .setAuthor({ name: `Recommended runes for ${champ.name} - ${lane}`, iconURL: `https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champion-icons/${champ.id}.png` })
        ...
        if (!replied) return interaction.followUp({ embeds: [error('en-US', 'runes-not-found', interaction.user.tag)] });
    }, 15000);
}
